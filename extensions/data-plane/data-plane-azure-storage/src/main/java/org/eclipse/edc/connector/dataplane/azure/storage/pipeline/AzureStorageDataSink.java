...
public class AzureStorageDataSink extends ParallelSink {
    ...
    protected StreamResult<Void> transferParts(List<DataSource.Part> parts) {
        monitor.debug("Starting to transfer " + parts.size() + " part(s)");
        for (DataSource.Part part : parts) {
            final var name = getDestinationBlobName(part.name());
            monitor.debug("Transferring part : " + part.name() + " to " + name);
            try {
                final var adapter = getAdapter(name);
                try (var input = part.openStream()) {
                    try (var output = adapter.getOutputStream()) {
                        try {
                            monitor.debug("Starting transfer to Azure Storage for part : " + part.name());
                            input.transferTo(output);
                        } catch (Exception e) {
                            return getTransferResult(e, "Error transferring blob for %s on account %s", name, accountName);
                        }
                    } catch (Exception e) {
                        return getTransferResult(e, "Error getting out stream for blob %s on account %s", name, accountName);
                    }
                    try {
                        monitor.debug("Updating metadata on Azure Storage for part : " + part.name());
                        adapter.setMetadata(metadataProvider.provideSinkMetadata(request, part).getMetadata());
                    } catch (Exception e) {
            ...
            monitor.debug("Successfully transferred part : " + part.name());
        }
